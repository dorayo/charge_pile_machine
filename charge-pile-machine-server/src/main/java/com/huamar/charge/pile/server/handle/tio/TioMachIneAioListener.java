...
public class TioMachIneAioListener implements ServerAioListener {
    ...
    public void onAfterConnected(ChannelContext channelContext, boolean isConnected, boolean isReconnect) {
        ...
        String connected = isConnected ? "成功" : "失败";
        String reconnect = isReconnect ? "是" : "否";
        int size = tioConfig.ids.getMap().size();
        log.info("有新的连接，当前总连接数：{}", size);
        log.info("有新的连接，id: {}，连接：{} ，是否重连：{}，bsId：{}", id, connected, reconnect, channelContext.getBsId());
    }
    ...
    public void onBeforeClose(ChannelContext channelContext, Throwable throwable, String remark, boolean isRemove) {
        TioConfig tioConfig = channelContext.getTioConfig();
        System.out.println("有用户断开连接，当前总连接数：" + (tioConfig.ids.getMap().size() - 1) + "，断开原因：" + remark + "，是否删除：" + (isRemove ? "是" : "否"));
    }
